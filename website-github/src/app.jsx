...
function SEO({ path }) {
  useEffect(() => {
    const titles = {
      ...
      "/commonplace": "Commonplace Book — Joe Holschuh",
...
function GenericContent({ section, onNav }) {
  if (section.type === "quotes") return (
    <>
      ...
      <ul className="quote-list">{section.quotes.slice(0, 5).map((q, i) => <li key={i}>{q}</li>)}</ul>
      <DrawerLinks links={[
        ...( section.links || [] ),
...
const qpCss = `
  ...
  .qp-item { padding: 1.4rem 0; border-bottom: 1px solid var(--border); }
  .qp-item:first-child { border-top: 1px solid var(--border); }
  .qp-quote { font-family: 'Inter', system-ui, sans-serif; font-size: 1rem; line-height: 1.8; color: var(--ink); font-style: italic; }
  @media (max-width: 600px) { .qp-quote { font-size: 0.92rem; } }
`;
...
export default function App() {
  ...
  const commonplaceSection = SECTIONS.find(s => s.slug === "commonplace");
  if (path === "/commonplace" && commonplaceSection) return <QuotesPage section={commonplaceSection} onNav={onNav} dark={dark} setDark={setDark} />;
